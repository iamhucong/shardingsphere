...
@Slf4j
public final class InventoryStreamingDumper extends AbstractInventoryDumper {
    
    public InventoryStreamingDumper(final InventoryDumperContext dumperContext, final PipelineChannel channel, final DataSource dataSource,
                                    final PipelineTableMetaDataLoader metaDataLoader, final InventoryDataRecordPositionCreator positionCreator) {
        super(dumperContext, channel, dataSource, metaDataLoader, positionCreator);
    }
    
    @Override
    protected void doRunBlocking(final IngestPosition position, final Connection connection) throws SQLException {
    ...
    private void fetchAllQuery(final Connection connection, final PipelineTableMetaData tableMetaData, final DatabaseType databaseType,
        ...
        try (PreparedStatement statement = JDBCStreamQueryBuilder.build(databaseType, connection, buildFetchAllSQLWithStreamingQuery(), batchSize)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                consumeResultSetToChannelAndFinished(tableMetaData, resultSet, batchSize);
            } finally {
                runningStatement.set(null);
            }
        }
    ...
    private void consumeResultSetToChannelAndFinished(final PipelineTableMetaData tableMetaData, final ResultSet resultSet, final int batchSize) throws SQLException {
        consumeResultSetToChannel(tableMetaData, resultSet, batchSize);
        sendFinishedRecordToChannel();
    }
    ...
    private void sendFinishedRecordToChannel() {
        channel.push(Collections.singletonList(new FinishedRecord(new IngestFinishedPosition())));
        log.info("Inventory dump done, dataSource={}, actualTable={}", dumperContext.getCommonContext().getDataSourceName(), dumperContext.getActualTableName());
    }
